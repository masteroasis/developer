...
- `site-starter/` — a self-contained, dependency-free scaffold for a *separate* client website project, parked here until it is exported to its own repo (`site-starter/README.md` documents the export, Cloudflare Pages setup, and GoDaddy DNS steps). It is **not** part of the Lakefront site and is not linked from `index.html`. Publishable files live in `site-starter/public/`; its `deploy.yml` uses `cloudflare/wrangler-action` (not the deprecated `pages-action`) and is inert while nested in a subdirectory, since GitHub only runs workflows from the repository root. Its contact form has no backend — `script.js` intercepts the submit until one is wired up.
