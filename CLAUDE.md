...
- `index.html` / `styles.css` / `script.js` — the Lakefront Technology Professional Services marketing site: a single-page layout with a 24/7 emergency bar, header/nav, hero, services grid, about, "why us", contact form, and footer. The emergency bar (`.emergency-bar`) sits above the sticky header and holds the red `tel:` call button (`.btn-emergency`) — it is deliberately not a `.reveal` element, so it paints immediately, and it is the page's first focusable element. All styling lives in `styles.css` (no CSS framework, no build step). `script.js` (vanilla JS, no dependencies) drives the footer year, scroll-reveal animations (`.reveal` / `.reveal.is-visible`, via `IntersectionObserver`), animated stat counters (`[data-count]`/`[data-suffix]`), the hero's mouse-parallax pine-tree scene (`.parallax-layer[data-depth]`), and the header's scrolled-shadow state — all gated behind a `prefers-reduced-motion` check in JS, with a matching CSS `@media (prefers-reduced-motion: reduce)` override as a fallback.
